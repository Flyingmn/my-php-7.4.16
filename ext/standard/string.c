...
static zend_always_inline zend_string *php_trim_int(zend_string *str, char *what, size_t what_len, int mode)
{
	...
	*/
	const char *start = ZSTR_VAL(str);//起始地址
	const char *end = start + ZSTR_LEN(str);//结束地址
	char mask[256];
	...
	if (what) {
		...
		if (what_len == 1) {
			...
			//大意为把what的每个字符的ASCII值在mask数组中设置为1
			php_charmask((unsigned char*)what, what_len, mask);

			if (mode & 1) {
				while (start != end) {
					//(unsigned char)*start == c：当前str的字符在mask中存在
